...
pub async fn get_amounts_out(
    ...
    let vm_candidates_raw = if state.vm_ready() {
        state
            .vm_state_store
            .matching_pools_by_addresses(&token_in_bytes, &token_out_bytes)
            .await
    } else {
        if state.enable_vm_pools {
            metrics.skipped_vm_unavailable = true;
        }
        Vec::new()
    };

    let mut native_candidates = Vec::with_capacity(native_candidates_raw.len());
    for (id, (pool_state, component)) in native_candidates_raw.into_iter() {
        native_candidates.push((id, pool_state, component));
    }

    let mut vm_candidates = Vec::with_capacity(vm_candidates_raw.len());
    for (id, (pool_state, component)) in vm_candidates_raw.into_iter() {
