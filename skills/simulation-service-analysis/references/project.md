...
- Cold starts can take several minutes (3–5+ mins; VM pools can take up to roughly 10 minutes on a fresh warmup).
...
- The analyzer starts or reuses the local server, waits for readiness, runs representative `/simulate` and `/encode` probes, executes latency and light stress sweeps, then writes artifacts under `logs/simulation-reports/`.
- Default output root:
